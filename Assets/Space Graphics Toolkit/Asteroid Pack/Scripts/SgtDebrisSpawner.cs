	...
	public class SgtDebrisSpawner_Editor : SgtEditor<SgtDebrisSpawner>
	{
		protected override void OnInspector()
		{
			...
			BeginError(Any(t => t.Shapes != null && t.Shapes.Contains(null) == true));
				DrawDefault("Shapes", "These shapes define where the debris can spawn, if this is empty then they will spawn everywhere.");
			EndError();
		}
	}
}
...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtDebrisSpawner : MonoBehaviour
	{
		...
		/// <summary>These shapes define where the debris can spawn, if this is empty then they will spawn everywhere.</summary>
		...
		{
			Gizmos.matrix = SgtHelper.Translation(Target != null ? Target.position : transform.position);
